...
;
 // May be empty, or it may contain: keyword PROGRAM, program name (IDENT), and a semicolon
...
DATA_TYPE_NAME: KW_INTEGER
   ...
   | KW_STRING
;
 // One of the following keywords: Integer, Real, Char, String
...
;
 // keyword PROCEDURE, function header (FUN_HEAD), semicolon, section list (SECTION_LIST), and block (BLOCK)
...
FORM_PARAMS: %empty
   | '(' FORM_PARAM_LIST ')'
;
 // Either empty, or a list of formal parameters (FORM_PARAM_LIST) in parentheses
...
NUMBER: INTEGER_CONST
   | FLOAT_CONST
;
 // Either an integer constant (INTEGER_CONST) or a real constant (FLOAT_CONST)

ASSIGN_INSTR: IDENT QUALIF ASSIGN EXPR { found("ASSIGN_INSTR", $1); }
;
 // identifier, qualifier (QUALIF), assignment operator (ASSIGN), and expression (EXPR)
...
;
 // keyword IF, logical expression LOG_EXPR, keyword THEN, instruction block BLOCK_INSTR, and else part (ELSE_PART)
...
LOG_EXPR: EXPR '<' EXPR
   ...
   | '(' LOG_EXPR ')' 
;
 // Either two expressions EXPR linked with <, >, LE, =, or a logical expression in parentheses*/

ELSE_PART: %empty
   | KW_ELSE BLOCK_INSTR
;
 // Either empty or keyword ELSE, and instruction block (BLOCK_INSTR) 
